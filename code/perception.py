...
def perception_step(Rover):
    ...
    # 1) Define source and destination points for perspective transform
     # source coordinates : actual el heyya fl walkthrough kanet the square msh koll el sides ad baad, 
     # destination coordinates : desired coordinates el howa el square 1m*1m
        # both coordinates used to get the bev of img
    ...
    bottom_offset = 6
     # values from actal grid zy fl walkthrough
    source = np.float32([[14, 140], [301 ,140],[200, 96], [118, 96]])
    
    # han calculate el actaul sizes 3ashan ngeeb el bev zy fl walkthrough bl offset wl dst.size 
    ...
    #koll el ones htb2a 255 : white 
    mask[:,:] = 255

    #apply mask to get bev
    ...
    # obstacles[0:int(obstacles.shape[0]/2),:] = 0
        ...
        #BGR not RGB 
        #hen taking lower and upper bounds by 10 points bellow and above 
        # thresholding bs between values  
    lower_yellow = np.array([24 - 5, 100, 100])
    upper_yellow = np.array([24 + 5, 255, 255])

            # Convert BGR to HSV for better processing 3ashan a segment objects based ala el color
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)

            # Threshold the HSV image to get only upper_yellow colors
            # inRange : detect objects based on hsv values produced
            #from cv2.cvtcolor 
    rock_samples = cv2.inRange(hsv, lower_yellow, upper_yellow)

    #ageeb el bev bta3 el rocks baad ma 3amalt detection 
    ...
    #channel for each segment in the img 
    ...
    # 7) Update Rover worldmap (to be displayed on right side of screen)
        ...
        #cleaned up the obstacle pixels that overlapped with navigable terrain measurements:
    ...
    # Update Rover pixel distances and angles
        ...
        # to_polar_coords :calculated the navigable terrain and rocks into a polar coordinate system
